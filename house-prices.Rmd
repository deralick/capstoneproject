...
logarithmicPCRPredict <- exp(logarithmicPCRPredict)
```
...
```
Looking at the above performance, it seems like the Linear model is outperforming the logarithmic when it comes to RMSE, but their R2 is near identical.

I will run a t-test on the two to compare if there is a statistical difference
